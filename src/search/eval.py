...
# ---------------------------------------------------------------------------
# Centipawn piece values (middlegame / endgame)
# ---------------------------------------------------------------------------
MG_PIECE_VALUE = {
    PieceType.PAWN: 82,
...
# Indexed [square] where square = rank*8 + file, from WHITE's perspective.
# For BLACK, mirror vertically: sq -> sq ^ 56.
# ---------------------------------------------------------------------------
...
# Lookup: PieceType -> (mg_table, eg_table)
_PST = {
...
def evaluate(board: Board) -> int:
    ...
    Computes separate middlegame and endgame scores, then interpolates
    based on game phase (piece material on board). Returns score in
    ...
    for sq, piece in enumerate(board.squares):
        ...
        # Map board square to table index.
        # PeSTO tables: index 0 = a8, index 63 = h1 (rank 8 at top).
        # Board squares: index 0 = a1, index 63 = h8 (rank 1 at bottom).
        # White: flip rank (sq ^ 56). Black: use sq directly (double-flip cancels).
        psq = (sq ^ 56) if color == Color.WHITE else sq

        mg_table, eg_table = _PST[pt]
        mg = MG_PIECE_VALUE[pt] + mg_table[psq]
        eg = EG_PIECE_VALUE[pt] + eg_table[psq]
