...
Computes the exact 64-bit Zobrist hash used by Polyglot opening books.
The keys are the official pre-computed values from the Polyglot specification.

Polyglot square indexing: a1 = 0, b1 = 1, ..., h1 = 7, a2 = 8, ..., h8 = 63
This matches: square = rank * 8 + file, where file: a=0..h=7, rank: 1=0..8=7

Polyglot piece indexing (for the Random64 array):
    black pawn   = 0
    white pawn   = 1
    black knight = 2
    white knight = 3
...
# =============================================================================
# Official Polyglot Random Numbers (781 values)
# =============================================================================
# Layout:
#   [0..767]   : piece_square keys (12 pieces * 64 squares)
#   [768..771] : castling keys (WK, WQ, BK, BQ)
#   [772..779] : en passant file keys (a..h)
...
POLYGLOT_RANDOM_ARRAY = (
    ...
    # Castling keys (768-771): WK, WQ, BK, BQ
    0x31d71dce64b2c310, 0xf165b587df898190, 0xa57e6339dd2cf3a0, 0x1ef6e6dbb1961ec9,
    # En passant file keys (772-779): a-h files
    0x70cc73d90bc26e24, 0xe21a6b35df0c3ad7, 0x003a93d8b2806962, 0x1c99ded33cb890a1,
    0xcf3145de0add4289, 0xd0e4427a5514fb72, 0x77c621cc9fb3a483, 0x67a34dac4356550b,
    # Turn key (780): XOR'd when white to move
    0xf8d626aaaf278509,
)
...
class PolyglotZobrist:
    """
    Provides access to the exact Polyglot Zobrist keys.
    
    Layout of the Random64 array (781 entries total):
        [0..767]   : piece_square keys (12 pieces * 64 squares)
        [768..771] : castling keys (WK, WQ, BK, BQ)
        [772..779] : en passant file keys (a..h)
        [780]      : side to move key (white to move)
    ...
    # Offsets into the Random64 array
    PIECE_OFFSET = 0        # 12 * 64 = 768 entries
    CASTLE_OFFSET = 768     # 4 entries
    EP_OFFSET = 772         # 8 entries  
    TURN_OFFSET = 780       # 1 entry
    
    def __init__(self):
        """Initialize with the exact Polyglot random keys."""
        # Pre-extract commonly used sections for clarity
        # piece_keys[piece_kind][square] where piece_kind is 0..11
        self.piece_keys: List[List[int]] = [
            [POLYGLOT_RANDOM_ARRAY[kind * 64 + sq] for sq in range(64)]
            for kind in range(12)
        ]
        
        # Castling keys: index 0=white kingside, 1=white queenside, 
        #                      2=black kingside, 3=black queenside
        self.castle_keys: List[int] = [
            POLYGLOT_RANDOM_ARRAY[self.CASTLE_OFFSET + i] for i in range(4)
        ]
        
        # En passant file keys (only used if there's actually an EP capture possible)
        self.ep_keys: List[int] = [
            POLYGLOT_RANDOM_ARRAY[self.EP_OFFSET + i] for i in range(8)
        ]
        
        # Side to move key (XOR'd in when white is to move)
        self.turn_key: int = POLYGLOT_RANDOM_ARRAY[self.TURN_OFFSET]
    
    def piece_key(self, color: int, piece_type: int, square: int) -> int:
        """
        ...
        Args:
            ...
            square: 0..63 in Polyglot format (a1=0, b1=1, ..., h8=63)
        
        Returns:
            The 64-bit Zobrist key component for this piece-square.
        """
        # Polyglot piece indexing:
        # black_pawn=0, white_pawn=1, black_knight=2, white_knight=3, etc.
        # So: piece_kind = (piece_type - 1) * 2 + (1 - color)
        # For white (color=0): piece_kind = (pt-1)*2 + 1 = odd numbers
        # For black (color=1): piece_kind = (pt-1)*2 + 0 = even numbers
        piece_kind = (piece_type - 1) * 2 + (1 - color)
        return self.piece_keys[piece_kind][square]
    
    def castling_key(self, castling_rights: int) -> int:
        """
        Get the combined Zobrist key for castling rights.
        
        Args:
            castling_rights: Bitmask where:
                bit 0 (1): white kingside (K)
                bit 1 (2): white queenside (Q)
                bit 2 (4): black kingside (k)
                bit 3 (8): black queenside (q)
        
        Returns:
            XOR of all applicable castling keys.
        """
        key = 0
        if castling_rights & 1:  # White kingside
            key ^= self.castle_keys[0]
        if castling_rights & 2:  # White queenside
            key ^= self.castle_keys[1]
        if castling_rights & 4:  # Black kingside
            key ^= self.castle_keys[2]
        if castling_rights & 8:  # Black queenside
            key ^= self.castle_keys[3]
        return key
    
    def en_passant_key(self, ep_file: Optional[int]) -> int:
        """
        Get the Zobrist key for en passant file.
        
        Args:
            ep_file: The file (0-7) of the en passant square, or None
        
        Returns:
            The EP file key if ep_file is set, else 0.
        """
        if ep_file is None:
            return 0
        return self.ep_keys[ep_file]
    
    def turn_key_if_white(self, is_white_to_move: bool) -> int:
        """
        Get the side-to-move key.
        
        In Polyglot, the turn key is XOR'd in only when WHITE is to move.
        
        Args:
            is_white_to_move: True if it's white's turn
        
        Returns:
            The turn key if white to move, else 0.
        """
        return self.turn_key if is_white_to_move else 0


# Pre-computed instance for efficiency
_POLYGLOT_ZOBRIST: Optional[PolyglotZobrist] = None


def get_polyglot_zobrist() -> PolyglotZobrist:
    """Get the singleton PolyglotZobrist instance."""
    global _POLYGLOT_ZOBRIST
    if _POLYGLOT_ZOBRIST is None:
        _POLYGLOT_ZOBRIST = PolyglotZobrist()
    return _POLYGLOT_ZOBRIST
...
def square_from_file_rank(file: int, rank: int) -> int:
    """
    Convert file and rank to Polyglot square index.
    ...
    Returns:
        Square index 0..63 where a1=0, b1=1, ..., h8=63
    """
    return rank * 8 + file
...
def algebraic_to_square(s: str) -> int:
...
# =============================================================================
# Hash computation for a full position
# =============================================================================

def compute_polyglot_hash(
    ...
    """
    Compute the full Polyglot Zobrist hash for a position.
    
    Args:
        squares: 64-element list where each element is None or (color, piece_type)
                 color: 0=white, 1=black
                 piece_type: 1=pawn, 2=knight, 3=bishop, 4=rook, 5=queen, 6=king
        side_to_move: 0 for white, 1 for black
        castling_rights: Bitmask (1=WK, 2=WQ, 4=BK, 8=BQ)
        ep_square: En passant target square (0-63) or None
    
    Returns:
        64-bit Polyglot Zobrist hash
    """
    zobrist = get_polyglot_zobrist()
    key = 0
    ...
    for sq in range(64):
        ...
        if piece is not None:
            ...
            key ^= zobrist.piece_key(color, piece_type, sq)
    
    # 2. Hash castling rights
    key ^= zobrist.castling_key(castling_rights)
    
    # 3. Hash en passant (only if capture is actually possible)
    if ep_square is not None:
        ...
        if side_to_move == 0:  # White to move, EP square is on rank 5 (index 5)
            # Check for white pawns on rank 4 (index 4) adjacent to EP file
            pawn_rank = 4
            for df in [-1, 1]:
                adj_file = ep_file + df
                if 0 <= adj_file <= 7:
                    adj_sq = square_from_file_rank(adj_file, pawn_rank)
                    piece = squares[adj_sq]
                    if piece is not None and piece[0] == 0 and piece[1] == 1:
                        can_capture = True
                        break
        else:  # Black to move, EP square is on rank 2 (index 2)
            # Check for black pawns on rank 3 (index 3) adjacent to EP file
            pawn_rank = 3
            for df in [-1, 1]:
                adj_file = ep_file + df
                if 0 <= adj_file <= 7:
                    adj_sq = square_from_file_rank(adj_file, pawn_rank)
                    piece = squares[adj_sq]
                    if piece is not None and piece[0] == 1 and piece[1] == 1:
                        can_capture = True
                        break
        
        if can_capture:
            key ^= zobrist.ep_keys[ep_file]
    
    # 4. Hash side to move (only if white)
    if side_to_move == 0:
...
def compute_polyglot_hash_from_board(board) -> int:
    """
    Compute Polyglot hash from a Board object.
    ...
    Returns:
        64-bit Polyglot Zobrist hash
    """
    # Convert board.squares to the expected format
    # Board squares are: List[Optional[Tuple[Color, PieceType]]]
    # We need: List[Optional[Tuple[int, int]]] where color and piece_type are ints
    
    squares = []
    for sq_data in board.squares:
        if sq_data is None:
            squares.append(None)
        else:
            color, piece_type = sq_data
            # Convert enums to ints if necessary
    ...
    return compute_polyglot_hash(squares, side_to_move, castling_rights, ep_square)
